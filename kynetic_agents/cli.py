...
def _systemd_unit_text(home: Path) -> str:
    ...
    return dedent(
        f"""\
        [Unit]
        Description=kynetic-agents agent ({home.name})
        ...
        ExecStart={uv_bin} run kynetic-agents run --home {home}
        Restart=on-failure
        RestartSec=5
...
def _launchd_label(home: Path) -> str:
    return f"ai.kynetic-agents.{_service_slug(home.name)}"
...
def _windows_task_name(home: Path) -> str:
    return f"KyneticAgents-{_service_slug(home.name)}"
...
def _windows_task_install_ps1(home: Path) -> str:
    ...
    return dedent(
        ...
        $HomeDir = "{home_value}"
        $Uv = "{uv_bin}"
        $Args = "run kynetic-agents run --home `"$HomeDir`""

        $Action = New-ScheduledTaskAction -Execute $Uv -Argument $Args
        $Trigger = New-ScheduledTaskTrigger -AtLogOn
        $Settings = New-ScheduledTaskSettingsSet -StartWhenAvailable -RestartCount 3 -RestartInterval (New-TimeSpan -Minutes 1)

        Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger $Trigger -Settings $Settings -Description "kynetic-agents agent" -Force | Out-Null
...
def _write_service_assets(home: Path) -> None:
    ...
    if platform == "linux":
        _write_if_missing(services_dir / "kynetic-agents.service", _systemd_unit_text(home))
        return
    if platform == "macos":
        _write_if_missing(
    ...
    if platform == "windows":
        _write_if_missing(
            services_dir / "install-kynetic-agents-task.ps1",
            _windows_task_install_ps1(home),
        )
        _write_if_missing(
            services_dir / "uninstall-kynetic-agents-task.ps1",
...
def _service_setup_section(home: Path) -> str:
    ...
    if platform == "linux":
        unit_path = services_dir / "kynetic-agents.service"
        if tools["systemctl"]:
            log_cmd = "journalctl --user -u kynetic-agents.service -f"
            if not tools["journalctl"]:
                log_cmd = "systemctl --user status kynetic-agents.service"
            return dedent(
                f"""\
                ...
                - install commands:
                  ...
                  - cp "{unit_path}" ~/.config/systemd/user/kynetic-agents.service
                  - systemctl --user daemon-reload
                  - systemctl --user enable --now kynetic-agents.service
                  - {log_cmd}
    ...
    if platform == "windows":
        install_script = services_dir / "install-kynetic-agents-task.ps1"
        uninstall_script = services_dir / "uninstall-kynetic-agents-task.ps1"
...
def _project_depends_on_kynetic_agents(pyproject_path: Path) -> bool:
...
def _ensure_uv_project(home: Path) -> None:
    ...
    if _project_depends_on_kynetic_agents(pyproject_path):
        return

    add_proc = _run_command(["uv", "add", "kynetic-agents"], cwd=home)
    if add_proc.returncode != 0:
        message = add_proc.stderr.strip() or add_proc.stdout.strip() or "unknown error"
        raise RuntimeError(f"uv add kynetic-agents failed: {message}")
...
def _ensure_git_identity(home: Path) -> None:
    ...
    if not sys.stdin.isatty():
        ...
        raise RuntimeError(
            ...
            "Run `kynetic-agents setup` interactively to provide git commit identity.".format(
...
def _ensure_initial_commit(home: Path) -> bool:
    ...
    commit_proc = _run_command(
        ["git", "commit", "-m", "Initial kynetic-agents scaffold"],
        cwd=home,
    )
...
def _ensure_git_remote(home: Path, *, github: bool = False, repo_name: str | None = None) -> None:
    ...
    print(
        "Git remote `origin` is missing. kynetic-agents auto-push needs a remote destination.",
        flush=True,
    )
    print(
...
def setup_home(home: Path, *, github: bool = False, repo_name: str | None = None) -> None:
    ...
    print(f"kynetic-agents setup complete: {home}", flush=True)
...
def _print_setup_walkthrough(home: Path) -> None:
    ...
    text = dedent(
        ...
        - if omitted, kynetic-agents starts normally but the `web_search` tool is disabled.

        3) Set up Discord bot (required)
        ...
        - home_channels: channel names or IDs this agent monitors (empty = respond everywhere)
        - api_port: loopback REST API port (0 disables it)

        5) Run
        - start agent: uv run kynetic-agents
        - requires a valid DISCORD_TOKEN in .env
...
def main(argv: Sequence[str] | None = None) -> None:
    parser = argparse.ArgumentParser(prog="kynetic-agents")
    subparsers = parser.add_subparsers(dest="command")

    run_parser = subparsers.add_parser("run", help="Run the kynetic-agents event loop.")
    run_parser.add_argument("--home", type=Path, default=None, help="Agent home directory (default: cwd).")

    setup_parser = subparsers.add_parser(
        "setup",
        help="Scaffold an kynetic-agents home (uv project + git + config + state + .env).",
    )
    ...
    if args.command in (None, "run"):
        ...
        run_kynetic_agents(home=run_home)
        return
