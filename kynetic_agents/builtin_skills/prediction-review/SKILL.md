...
- **Example:** "kynetic-agents announcement will happen today" → FALSE (Tim decided it wasn't ready — external decision you couldn't observe)
...
- `.kynetic_agents_builtin_skills/scripts/prediction_review_log.py`

Canonical command:

```bash
uv run python .kynetic_agents_builtin_skills/scripts/prediction_review_log.py \
...
5. Log the result with `.kynetic_agents_builtin_skills/scripts/prediction_review_log.py`.
...
- **Announcements depend on human decisions:** "kynetic-agents announcement will happen" → FALSE (Tim decided it wasn't ready). External timing you can't observe.
