...
The memory skill (`/.kynetic_agents_builtin_skills/memory/SKILL.md`) covers:
...
The file frequency report (`/.kynetic_agents_builtin_skills/scripts/file_frequency_report.py`)
...
- **Scheduled job issues?** → Read `/.kynetic_agents_builtin_skills/introspection/debugging-jobs.md`
  Covers: job not firing, firing at wrong time, cron vs time_of_day, timezone traps,
  validation errors, prompt failures

- **Communication pattern issues?** → Read `/.kynetic_agents_builtin_skills/introspection/debugging-communication.md`
...
- **Behavioral drift after model changes or block edits?** → Read `/.kynetic_agents_builtin_skills/introspection/debugging-drift.md`
...
- **Identity or operational drift?** → Read `/.kynetic_agents_builtin_skills/onboarding/SKILL.md`
