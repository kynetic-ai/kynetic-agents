...
Hooks are command-line extensions declared by skills. They let a skill observe or modify runtime events without changing kynetic-agents core code.
...
| `pre_prompt` | After kynetic-agents renders the turn prompt, before the LLM is invoked. | `prompt`, `append_prompt` |
| `pre_startup` | Early in `OpenStrixApp.run()`, before MCP, scheduler, and APIs start. | No runtime effect |
| `post_startup` | After scheduler/API startup and `app_started`, before Discord/stdin blocking mode. | No runtime effect |
...
For each matching hook, kynetic-agents starts the hook command with:
...
The command receives exactly one JSON object plus a newline on stdin. If it writes a JSON object to stdout, that object replaces the event for the next hook in the chain. If it writes nothing to stdout, kynetic-agents treats the hook as a no-op.
...
When `logs/chat-history.jsonl` exists, `all_messages` is loaded from that persisted chat transcript and `channel_messages` is filtered from it. During very early startup or in tests without the log file, kynetic-agents falls back to the in-memory conversation buffers.
...
If the hook returns a JSON object with an `args` object, those args are used for the actual tool call. `args` must remain a JSON object. If it does not, kynetic-agents logs `hook_invalid_mutation` and keeps the original args.
...
Use lifecycle hooks for local setup and teardown: starting sidecar processes, writing readiness files, registering local OS affordances, or flushing metrics.
...
- Each hook command is short-lived. Long-running daemons should be supervisor jobs, OS services, or pollers.
